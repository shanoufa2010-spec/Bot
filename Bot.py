...
# 🔒 قاعدة بيانات الإعدادات الافتراضية
BOT_CONFIG = {
    "web_user": "admin",
...
# 💾 نظام الحفظ الذكي للحسابات لضمان عدم حذفها عند إعادة تشغيل البوت
...
# 🔒 صفحة تسجيل الدخول السرية للإدارة
LOGIN_HTML = """
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
    ...
    <style>
        ...
        h2 { color: #ed4245; margin-bottom: 25px; }
        ...
        button { width: 100%; padding: 12px; background: #ed4245; border: none; color: white; font-size: 16px; font-weight: bold; border-radius: 6px; cursor: pointer; }
...
<head>
    ...
    <style>
        ...
        .sidebar h2 { color: #5865F2; text-align: center; margin-bottom: 30px; }
        .sidebar a { display: block; padding: 12px; color: #a2a0b6; text-decoration: none; border-radius: 6px; margin-bottom: 10px; }
        .sidebar a.active { background: #231b3e; color: #fff; border-right: 4px solid #5865F2; }
        .main-content { margin-right: 260px; padding: 40px; width: 100%; box-sizing: border-box; }
        ...
        input[type="text"], select { padding: 12px; background: #0f0c1b; border: 1px solid #2c254a; color: #fff; border-radius: 6px; }
        .btn-save { background: #43b581; color: white; border: none; padding: 14px; font-weight: bold; border-radius: 6px; cursor: pointer; width: 100%; }
        .btn-danger { background: #ed4245; color: white; padding: 6px 10px; text-decoration: none; border-radius: 4px; font-size: 12px; }
        .acc-table { width: 100%; border-collapse: collapse; margin-top: 15px; }
        .acc-table th, .acc-table td { padding: 12px; text-align: right; border-bottom: 1px solid #2c254a; }
        .acc-table th { color: #5865F2; }
...
<body>
    <div class="sidebar">
        <h2>LTB Admin Panel ⚙️</h2>
        <a href="#" class="active">🎛️ لوحة التحكم والإضافة</a>
        <a href="/logout" class="logout">🚪 تسجيل الخروج الآمن</a>
    </div>
    
    <div class="main-content">
        <h2>🎛️ إدارة مخزن الحسابات والخيارات السرية (محفوظة دائماً)</h2>
        
        <div class="card">
            <h3>📦 إضافة وتعديل كروت الحسابات في المتجر العام</h3>
            <form method="POST" action="/add_account" style="margin-bottom: 20px;">
                <div class="form-row">
                ...
                <button type="submit" style="background:#5865F2; color:white; border:none; padding:10px 20px; border-radius:6px; cursor:pointer;">➕ إضافة الحساب فوراً في الكتالوج العام</button>
            </form>
        ...
        <form method="POST" action="/save">
            ...
            <button type="submit" class="btn-save">💾 حفظ التحديثات وتطبيقها على البوت</button>
        </form>
    </div>
</body>
</html>
...
def login():
    ...
    return render_template_string(LOGIN_HTML, error="❌ عذراً، خطأ في بيانات الأدمن السرية!")
        ...
        if target_acc in DB_ACCOUNTS:
            ...
            try:
                ...
                if log_channel:
                    ...
                    embed_p.set_image(url=acc_data['img_url'])
                    ...
                    active_transactions[interaction.channel.id] = {"log_msg_id": l_msg.id, "account_id": target_acc}
            except: pass
        else:
...
async def on_ready():
    ...
    print("⚡ LTB Web Manager Engine Enabled successfully with Secure JSON Database!")
